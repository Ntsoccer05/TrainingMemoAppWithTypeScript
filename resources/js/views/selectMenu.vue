<template>
  <div>
        ...
        <div class="text-right mr-2"><i class="fa-solid fa-trash "></i><span>：削除</span></div>
        <div class="md:flex">
            <table v-for="(category, index) in categories" :key="category.id" class="border border-collapse table-fixed mx-auto mt-3 md:w-5/12 w-11/12">
                ...
                <tbody>
                    <tr v-for="menu in categories[index].menus" :key="menu.id">
                        <td class="border hover:bg-gray-200" @click="toRecordContents(category.id, menu)"><span class="flex items-center">{{ menu.content }}<i class="fa-solid fa-pen ml-auto"></i><i class="fa-solid fa-trash ml-1 mr-1"></i></span></td>
                    </tr>
                </tbody>
            </table>
        </div>
